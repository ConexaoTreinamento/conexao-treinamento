...
export const mapStudentResponseToForm = (
  ...
  return {
    ...
    isDoctorAwareOfPhysicalActivity:
      student.anamnesis?.isDoctorAwareOfPhysicalActivity ?? undefined,
    favoritePhysicalActivity:
      student.anamnesis?.favoritePhysicalActivity ?? undefined,
    ...
    difficultiesInPhysicalActivities:
      student.anamnesis?.difficultiesInPhysicalActivities ?? undefined,
    medicalOrientationsToAvoidPhysicalActivity:
      student.anamnesis?.medicalOrientationsToAvoidPhysicalActivity ??
      undefined,
    surgeriesInTheLast12Months:
      student.anamnesis?.surgeriesInTheLast12Months ?? undefined,
    respiratoryProblems: student.anamnesis?.respiratoryProblems ?? undefined,
    jointMuscularBackPain:
      student.anamnesis?.jointMuscularBackPain ?? undefined,
...
const mapPhysicalImpairments = (
  ...
  const mapped = formData.physicalImpairments.reduce<
    PhysicalImpairmentRequestDto[]
  >((acc, item) => {
    if (!item) {
      return acc;
    ...
    const hasAnyValue =
      hasValue(item.type) || hasValue(item.name) || hasValue(item.observations);
    if (!hasAnyValue) {
      return acc;
    }
...
const hasAnamnesisData = (formData: StudentFormData) =>
  ANAMNESIS_FIELDS.some((field) =>
    hasValue((formData as Record<string, unknown>)[field as string]),
  );
