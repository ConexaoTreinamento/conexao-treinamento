...
import { Users, Calendar, Activity, TrendingUp, AlertTriangle, Plus, UserCheck, CalendarDays } from "lucide-react"
...
const EXPIRED_PLANS = [
  { id: "plan-1", student: "Ana Costa", plan: "Mensal", expiredLabel: "Vencido há 2 dias" },
  { id: "plan-2", student: "Carlos Lima", plan: "Trimestral", expiredLabel: "Vencido há 5 dias" },
  { id: "plan-3", student: "Lucia Ferreira", plan: "Mensal", expiredLabel: "Vencido há 1 dia" },
  { id: "plan-4", student: "Roberto Silva", plan: "Semestral", expiredLabel: "Vencido há 3 dias" },
  { id: "plan-5", student: "Patricia Oliveira", plan: "Mensal", expiredLabel: "Vencido há 1 semana" },
]

const PROFESSOR_SCHEDULE = [
  {
    id: "schedule-1",
...
export default function Dashboard() {
  ...
  const filteredActivities = useMemo(() => {
    if (!normalizedSearch) return RECENT_ACTIVITIES
    return RECENT_ACTIVITIES.filter((activity) => activity.title.toLowerCase().includes(normalizedSearch))
  }, [normalizedSearch])

  const filteredPlans = useMemo(() => {
    if (!normalizedSearch) return EXPIRED_PLANS
    return EXPIRED_PLANS.filter((plan) => plan.student.toLowerCase().includes(normalizedSearch))
  }, [normalizedSearch])
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        <DashboardSearch value={searchTerm} onChange={setSearchTerm} onClear={() => setSearchTerm("")} />

        {resultsSummary ? <p className="text-sm text-muted-foreground">{resultsSummary}</p> : null}
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          <DashboardActivitiesCard
            ...
            badge={normalizedSearch ? String(filteredActivities.length) : undefined}
          />

          {userRole === "admin" ? (
            <DashboardPlansCard
              plans={filteredPlans}
              badge={normalizedSearch ? String(filteredPlans.length) : String(EXPIRED_PLANS.length)}
        ...
        {userRole === "admin" ? (
          <div className="flex items-center gap-2 rounded-lg border border-orange-200 bg-orange-50 p-4 text-sm text-orange-800 dark:border-orange-900 dark:bg-orange-950/40 dark:text-orange-200">
            ...
            Mantenha o acompanhamento dos planos vencidos para evitar interrupções na rotina dos alunos.
          </div>
        ) : null}
      </div>
    </Layout>
