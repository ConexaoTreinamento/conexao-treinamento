...
function ClassDetailPageContent() {
  ...
  useQuery({
    ...getScheduleOptions({
      client: apiClient,
      query: { startDate: hintedDate || "", endDate: hintedDate || "" },
    }),
  ...
  const allExercises: ExerciseResponseDto[] =
    (exercisesQuery.data as PageExerciseResponseDto | undefined)?.content ?? [];
  ...
  const togglePresence = async (sid: string) => {
    ...
    const current =
      (session.students ?? []).find((p) => p.studentId === sid)?.present ??
      true;
    await mPresence.mutateAsync({
      client: apiClient,
      path: { sessionId: session.sessionId!, studentId: sid },
  ...
  const removeStudent = async () => {
    if (!session || !confirmationDialog.studentId) return;
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      ...
      return {
        ...old,
        students: (old.students ?? []).filter(
          (s) => s.studentId !== confirmationDialog.studentId,
        ),
      };
    });
    ...
    try {
      ...
      // Fecha o diálogo após a remoção bem-sucedida
      setConfirmationDialog({
        open: false,
        studentId: null,
        studentName: "",
  ...
  const addToExercisesCaches = (ex: ExerciseResponseDto) => {
    const entries = qc.getQueriesData<
      import("@/lib/api-client").PageExerciseResponseDto
    >({
      predicate: (q) => {
        const k0 = (q.queryKey as any)?.[0];
  ...
  const createExerciseAndUse = createExerciseForm.handleSubmit(
    async (values) => {
  ...
  const deleteExercise = async (exerciseRecordId: string) => {
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      ...
      const students = (old.students ?? []).map((s) => ({
        ...s,
        participantExercises: (s.participantExercises ?? []).filter(
          (ex) => ex.id !== exerciseRecordId,
        ),
      }));
      return { ...old, students };
    });
    await mRemoveExercise.mutateAsync({
      client: apiClient,
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      ...
      const students = (old.students ?? []).map((p) =>
        p.studentId === studentId
          ? {
              ...p,
              participantExercises: (p.participantExercises ?? []).map((ex) =>
                ex.id === exerciseRecordId ? { ...ex, done: !currentDone } : ex,
              ),
            }
          : p,
      );
  ...
  const excludedIds = new Set(
    (students || []).map((p) => p.studentId).filter(Boolean) as string[],
  );
  ...
  return (
    <Layout>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {session.endTime ? ` - ${session.endTime.slice(11, 16)}` : ""}
            </p>
          </div>
        </div>
          ...
          <Card>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    ...
                    className="h-8 px-2"
                    aria-label={
                      session.canceled ? "Restaurar aula" : "Cancelar aula"
                    }
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <span className="w-4 h-4 inline-flex items-center justify-center rounded-full bg-muted text-[10px]">
                    P
                  </span>
                  <span>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                {filteredStudents.map((student) => (
                  ...
                  >
                    <div
                      className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${student.participantExercises?.length ? "mb-3" : ""}`}
                    >
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
                        <div className="flex gap-2 w-full sm:w-auto">
                          <Button
                            ...
                            onClick={() =>
                              student.studentId &&
                              togglePresence(student.studentId)
                            }
                            className={`w-full sm:w-28 h-8 text-xs ${student.present ? "bg-green-600 hover:bg-green-700" : "border-red-300 text-red-600 hover:bg-red-50 dark:border-red-600 dark:text-red-400 dark:hover:bg-red-950"}`}
                          >
                            {student.present ? (
                              <>
                                <CheckCircle className="w-3 h-3 mr-1" />
                          ...
                          <Button
                            ...
                            onClick={() =>
                              student.studentId &&
                              openExerciseDialog(student.studentId)
                            }
                        ...
                        <Button
                          ...
                          className="hidden sm:flex h-8 w-8 p-0 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950 flex-shrink-0"
                          aria-label="Remover aluno da aula"
                          title="Remover"
                        >
                          <X className="w-4 h-4" />
                            ...
                            .map((ex) => (
                              ...
                              >
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  ...
                                  />
                                  <span
                                    className={`flex-1 min-w-0 truncate ${ex.done ? "line-through opacity-70" : ""}`}
                                  >
                                    {ex.exerciseName || ex.exerciseId}{" "}
                                    {ex.setsCompleted != null &&
                                      `- ${ex.setsCompleted}x${ex.repsCompleted ?? ""}`}{" "}
                                    {ex.weightCompleted != null &&
                                      `- ${ex.weightCompleted}kg`}
                                  </span>
                                </div>
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  onClick={() => ex.id && deleteExercise(ex.id)}
                                  className="h-6 w-6 p-0 flex-shrink-0 ml-2 text-red-500"
                                >
                                  <X className="w-4 h-4" />
                                </Button>
                              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-48 overflow-y-auto border rounded">
                    {(filteredExercises || []).map((ex) => {
                      const selected =
                        registerExerciseForm.watch("exerciseId") === ex.id;
                      return (
                        <button
                          key={ex.id}
                          type="button"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    placeholder="3"
                    {...registerExerciseForm.register("sets")}
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    {...registerExerciseForm.register("reps")}
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    step="0.5"
                    placeholder="20"
                    {...registerExerciseForm.register("weight")}
                  />
                </div>
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  {...registerExerciseForm.register("notes")}
                />
              </div>
            </div>
        ...
        >
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  {...createExerciseForm.register("name", { required: true })}
                />
              </div>
              <div className="space-y-1">
            ...
            <DialogFooter>
              ...
              <Button
                onClick={createExerciseAndUse}
                disabled={!createExerciseForm.watch("name")?.trim()}
                className="bg-green-600 hover:bg-green-700"
              >
                <Save className="w-4 h-4 mr-2" />
                Criar e Usar
      ...
      <ConfirmationDialog
        ...
        description={`Tem certeza que deseja remover ${confirmationDialog.studentName} desta aula? Esta ação não pode ser desfeita.`}
        confirmText="Remover"
        cancelText="Cancelar"
        variant="destructive"
      />
