...
import { Clock, Loader2, Save, Settings2, Square, SquareCheck } from "lucide-react"
            ...
            {weekConfig.map((row) => {
              ...
              return (
                ...
                >
                  <div className="mb-2">
                    <div className="flex items-center gap-2">
                      <Checkbox checked={row.enabled} onCheckedChange={() => onToggleWeekday(row.weekday)} />
                      <span className="text-[13px] font-medium">{weekdayLabel(row.weekday)}</span>
                    </div>
                  ...
                  {row.enabled ? (
                    <div className="space-y-2">
                      <div className="grid grid-cols-1 gap-2 text-[11px]">
                        <div>
                          <label className="mb-0.5 block text-[10px] font-medium">Série</label>
                          <Input
                            value={row.seriesName}
                            onChange={(event) => onUpdateWeekday(row.weekday, { seriesName: event.target.value })}
                            className="h-8 text-xs"
                            placeholder={DEFAULT_SERIES_NAME}
                          />
                        </div>
                        <div>
                          <label className="mb-0.5 block text-[10px] font-medium">Início do turno</label>
                          <Input
                            type="time"
                            value={row.shiftStart}
                            onChange={(event) => onUpdateWeekday(row.weekday, { shiftStart: event.target.value })}
                            className="h-8 text-xs"
                          />
                        </div>
                        <div>
                          <label className="mb-0.5 block text-[10px] font-medium">Fim do turno</label>
                          <Input
                            type="time"
                            value={row.shiftEnd}
                            onChange={(event) => onUpdateWeekday(row.weekday, { shiftEnd: event.target.value })}
                            className="h-8 text-xs"
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {slots.length === 0 ? (
                          <span className="text-xs text-muted-foreground">Sem blocos no horário informado.</span>
                        ) : (
                          slots.map((start) => {
                            ...
                            return (
                              ...
                              >
                                {selected ? <SquareCheck className="h-3 w-3" /> : <Square className="h-3 w-3" />}
                                {start}–{end}
                              </button>
                            )
                          })
