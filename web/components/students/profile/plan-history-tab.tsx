...
export function StudentPlanHistoryTab({ entries, isLoading, onAssignPlan }: StudentPlanHistoryTabProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        ...
        <div className="space-y-2 max-h-72 overflow-auto pr-1">
          {entries.map((entry) => (
            <div key={entry.id} className="p-3 rounded border flex items-center justify-between text-xs bg-muted/50">
              <div className="space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {entry.isExpiringSoon && !entry.isExpired && (
                    <Badge className="bg-yellow-500/20 text-yellow-700 dark:text-yellow-300" variant="secondary">
                      Expira em breve
                    </Badge>
                  )}
                  {entry.isExpired && <Badge variant="destructive">Expirado</Badge>}
                </div>
