          ...
          {hasAnamnesis ? (
            <>
              <div>
                ...
                <p className="text-sm">{student.anamnesis?.medication || "Nenhuma"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Médico ciente da prática de atividade física?</span>
                <p className="text-sm">{student.anamnesis?.isDoctorAwareOfPhysicalActivity ? "Sim" : "Não"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Atividade física favorita:</span>
                <p className="text-sm">{student.anamnesis?.favoritePhysicalActivity || "N/A"}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm">{student.anamnesis?.dietOrientedBy || "Não faz dieta orientada"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Problemas cardíacos:</span>
                <p className="text-sm">{student.anamnesis?.cardiacProblems || "Nenhum"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Hipertensão:</span>
                <p className="text-sm">{student.anamnesis?.hasHypertension ? "Sim" : "Não"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Doenças crônicas:</span>
                <p className="text-sm">{student.anamnesis?.chronicDiseases || "Nenhuma"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Dificuldades para realização de exercícios físicos?</span>
                <p className="text-sm">{student.anamnesis?.difficultiesInPhysicalActivities || "Nenhuma"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Orientação médica impeditiva de alguma atividade física?</span>
                <p className="text-sm">{student.anamnesis?.medicalOrientationsToAvoidPhysicalActivity || "Nenhuma"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Cirurgias nos últimos 12 meses:</span>
                <p className="text-sm">{student.anamnesis?.surgeriesInTheLast12Months || "Nenhuma"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Problemas respiratórios:</span>
                <p className="text-sm">{student.anamnesis?.respiratoryProblems || "Nenhum"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Dor muscular/articular/dorsal:</span>
                <p className="text-sm">{student.anamnesis?.jointMuscularBackPain || "Nenhuma"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Problemas de disco espinhal:</span>
                <p className="text-sm">{student.anamnesis?.spinalDiscProblems || "Nenhum"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Diabetes:</span>
                <p className="text-sm">{student.anamnesis?.diabetes || "Não"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Fumante:</span>
                <p className="text-sm">{student.anamnesis?.smokingDuration || "Não fumante"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Colesterol alterado:</span>
                <p className="text-sm">{student.anamnesis?.alteredCholesterol ? "Sim" : "Não"}</p>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Osteoporose (localização):</span>
                <p className="text-sm">{student.anamnesis?.osteoporosisLocation || "Não"}</p>
              </div>
            </>
          ) : (
            <p className="text-sm text-muted-foreground">Nenhuma informação de anamnese cadastrada.</p>
          )}
        </CardContent>
      </Card>

      {student.physicalImpairments && student.physicalImpairments.length > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-3">
            {student.physicalImpairments.map((impairment, index) => {
              const impairmentTypeLabel = impairment.type
                ? impairmentTypeOptions?.[impairment.type] ?? impairment.type
                : "N/A"
                ...
                <div key={impairment.id ?? index} className="p-3 border rounded-lg">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      ...
                      <p className="text-sm font-medium">{impairmentTypeLabel}</p>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Nome:</span>
