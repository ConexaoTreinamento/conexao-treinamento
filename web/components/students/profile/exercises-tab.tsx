...
export function StudentExercisesTab({ records, isLoading }: StudentExercisesTabProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        <div className="space-y-4">
          {isLoading && <p className="text-xs text-muted-foreground">Carregando exercícios...</p>}
          ...
          {records.map((record) => (
            <div key={record.key} className="p-4 rounded-lg border bg-muted/50">
              <div className="mb-3">
                <h3 className="font-medium">{record.className} - {record.instructor}</h3>
                <p className="text-xs text-muted-foreground">{record.classDateLabel}</p>
              </div>
