...
export default function EventModal({
  ...
  const { control, handleSubmit, watch, setValue, reset, getValues, register } =
  ...
  const handleTimeChange = (
    ...
    const hasCompleteStart =
      (field === "startTime" ? value : otherValue)?.length === 5;
    const hasCompleteEnd =
      (field === "endTime" ? value : otherValue)?.length === 5;
  ...
  const addParticipant = (student: StudentSummary): boolean => {
    ...
    setValue("participantIds", [...ids, student.id], {
      shouldDirty: true,
      shouldValidate: true,
    });
            ...
            >
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Escolha um aluno da lista paginada.
                  </DialogDescription>
                </DialogHeader>
                <StudentPicker
              ...
              <div className="border rounded-lg p-3 max-h-40 overflow-y-auto space-y-2">
                {participantIds.map((studentId) => {
                  ...
                  const displayName = summary
                    ? `${summary.name ?? ""} ${summary.surname ?? ""}`.trim()
                    : "";
