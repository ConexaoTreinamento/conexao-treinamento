...
export function SessionDetailView({ sessionId, hintedDate, hintedTrainer }: SessionDetailViewProps) {
  ...
  const invalidateScheduleForSessionMonth = () => {
    ...
    qc.invalidateQueries({ queryKey: scheduleByDateQueryOptions({ startDate: toIso(monthStart), endDate: toIso(monthEnd) }).queryKey })
    ...
    qc.invalidateQueries({
      predicate: (q) => {
        const root = (q.queryKey as unknown[])?.[0] as { _id?: string } | undefined
  ...
  const invalidateSessionQueries = () => {
    ...
    qc.invalidateQueries({
      predicate: (q) => {
        const root = (q.queryKey as unknown[])?.[0] as { _id?: string; path?: { sessionId?: string } } | undefined
        return !!root && typeof root === "object" && root._id === "getSession" && root.path?.sessionId === sessionId
      },
    })
    ...
    qc.setQueryData<SessionResponseDto>(key, (old) => {
      ...
      const students = (old.students ?? []).map((student) =>
        student.studentId === studentId
          ? ({ ...student, present: !(student.present ?? (student.commitmentStatus === "ATTENDING")) })
          : student,
      )
      return { ...old, students }
    })

    const current = session.students?.find((student) => student.studentId === studentId)?.present ?? true
    try {
      await mPresence.mutateAsync({ path: { sessionId: session.sessionId!, studentId }, body: { present: !current } })
    } catch {
  ...
  const removeStudent = async (studentId: string) => {
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      if (!old) return old
      return { ...old, students: (old.students ?? []).filter((s) => s.studentId !== studentId) }
    })
    await mRemoveParticipant.mutateAsync({ path: { sessionId: session.sessionId!, studentId } })
    invalidateSessionQueries()
  }

  const trainers: TrainerResponseDto[] = useMemo(() => (trainersQuery.data as TrainerResponseDto[] | undefined) ?? [], [trainersQuery.data])
  ...
  const handleAddStudent = async (student: StudentSummary) => {
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      if (!old) return old
      const exists = (old.students ?? []).some((participant) => participant.studentId === studentId)
    ...
    try {
      await mPresence.mutateAsync({ path: { sessionId: session.sessionId!, studentId }, body: { present: true } })
    } catch {}
    invalidateSessionQueries()
  ...
  const addToExercisesCaches = (exercise: ExerciseResponseDto) => {
    const entries = qc.getQueriesData<PageExerciseResponseDto>({
      predicate: (query) => {
        ...
        return !!root && typeof root === "object" && root._id === "findAllExercises"
      },
    })
  ...
  const deleteExercise = async (exerciseRecordId: string) => {
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      ...
      const students = (old.students ?? []).map((participant) => ({
        ...participant,
        participantExercises: (participant.participantExercises ?? []).filter((exercise) => exercise.id !== exerciseRecordId),
      }))
      return { ...old, students }
    })
    ...
    qc.setQueryData<SessionResponseDto>(sessionOptions.queryKey, (old) => {
      ...
      const students = (old.students ?? []).map((participant) =>
        participant.studentId === studentId
          ? ({
              ...participant,
              participantExercises: (participant.participantExercises ?? []).map((exercise) =>
                exercise.id === exerciseRecordId ? { ...exercise, done: !currentDone } : exercise,
              ),
            })
          : participant,
      )
  ...
  const selectedExercise = useMemo(() => {
    ...
    return (allExercises || []).find((exercise) => exercise.id === selectedExerciseId) ?? null
  ...
  const selectedStudentName = selectedStudentId ? students.find((s) => s.studentId === selectedStudentId)?.studentName || "" : ""
  ...
  const excludedIds = new Set((students || []).map((participant) => participant.studentId).filter(Boolean) as string[])
  ...
  const filteredExercises = (allExercises || []).filter((exercise) => (exercise.name || "").toLowerCase().includes(normalizedExerciseSearch))
          ...
          <SessionParticipantsCard
            ...
            onDeleteExercise={(exerciseRecordId: string) => {
              ...
              for (const participant of students) {
                const exercise = (participant.participantExercises ?? []).find((ex) => ex.id === exerciseRecordId)
                if (exercise) {
                  exerciseName = exercise.exerciseName || exercise.exerciseId || exerciseName
                  break
                }
              }
