      ...
      <Card className={cn("transition-colors", muted ? "opacity-80" : "")}>
        <CardContent className="space-y-6 p-6">
          <div
            className={cn(
              "flex flex-col gap-4",
              hasAvatar ? "sm:flex-row sm:items-start" : "",
            )}
          >
            {hasAvatar ? (
              <div className="flex justify-center sm:block">{avatarNode}</div>
            ) : null}
            <div
              className={cn(
          ...
          {actionNodes.length ? (
            <div className="flex justify-center">
              <div className="flex w-full max-w-6xl flex-col gap-3 sm:grid sm:grid-cols-[repeat(auto-fit,minmax(12rem,1fr))] sm:gap-4 sm:mx-auto">
