...
export function AdministratorProfileSummaryCard({
  ...
  const displayName =
    administrator.fullName ?? (fallbackName || "Administrador");
  ...
  const createdAt =
    ...
    (administrator as { joinDate?: string }).joinDate ??
    null;
  ...
  const badges: ReactNode[] = [<StatusBadge key="status" active={isActive} />];
  ...
  return (
    <EntityProfile
      ...
      avatar={{
        label: initials || "?",
        backgroundClassName: "bg-blue-100 dark:bg-blue-900",
