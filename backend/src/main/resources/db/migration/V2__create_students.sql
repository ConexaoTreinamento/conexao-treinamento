CREATE TYPE insomnia_frequency AS ENUM ('Não', 'Sim', 'Às vezes', 'Raramente');
...
CREATE TABLE ANAMNESIS (
    ...
    IS_DOCTOR_AWARE_OF_PHYSICAL_ACTIVITY BOOLEAN DEFAULT FALSE,
    FAVORITE_PHYSICAL_ACTIVITY VARCHAR(255),
    HAS_INSOMNIA insomnia_frequency DEFAULT 'Não',
...
CREATE TABLE PHYSICAL_IMPAIRMENTS (
    ID VARCHAR(36) PRIMARY KEY,
    ANAMNESIS_ID VARCHAR(36) NOT NULL,
    IMPAIRMENT_TYPE physical_impairment_type NOT NULL,
    NAME VARCHAR(255) NOT NULL,
    OBSERVATIONS TEXT,
    CREATED_AT TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT FK_PHYSICAL_IMPAIRMENTS_ANAMNESIS FOREIGN KEY (ANAMNESIS_ID) REFERENCES ANAMNESIS (ID) ON DELETE CASCADE
);
...
CREATE INDEX IDX_PHYSICAL_IMPAIRMENTS_ANAMNESIS_ID ON PHYSICAL_IMPAIRMENTS (ANAMNESIS_ID);
